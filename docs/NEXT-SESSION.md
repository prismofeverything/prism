...
- 🧩 #34 north star: run streaming env with a hand-constructed Cell. **Mechanism complete; the Cell-scale demo is now just "exercise the mechanism more."**  
  ...
  - ⏳ **REMAINING (more, not new mechanism)**: hand-build the actual Cell from `cell.ys` — same shape, just verbose (BinOp/If bodies for Grow/Divide, more ports, bridges). The mechanism is in place; could go: write a `.ys` library of AST-builder helpers (`def add(a, b) = {_type: 'BinOp', op: 'Add', lhs: a, rhs: b}`) so the hand-construction reads like an EDSL.
