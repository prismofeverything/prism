...
- 🧩 #43 cross-composite reactor — **FIRST SLICE DONE 2026-06-08b** (the CRUX delta-returning reactor + engine per-tick reactor + auto-detect); the #40 link-graph surface matcher + chrysalis wiring + distributed form remain. The arc:
  ...
  - ⏳ **REMAINING**: (a) the SURFACE for the in-place form — `{ ?west: … }` needs `?`-prefixed reactum MAP KEYS in the parser (`parse_braces` only takes ident/string keys today) + a way to express the modified value (rest-capture, or composite methods #30 for `?west.balance(~e)`). The firing mechanism is done; this is sugar. (a') the SORTLESS sugar `?west ~{edge:~e}` (no `:: Sort`) — needs an `Expr::Site` ports field for faithful unparse; the sorted form is canonical, so optional. (b) restriction (2): a `?v` port target that BINDS the value. (c) the distributed form via #42 over a real bridge. (d) LIVE sub-engine composites — make `config.state` the live source for the (place-graph) reactor (today's engine test uses inert composite specs to isolate the mechanism).
