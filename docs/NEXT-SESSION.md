...
- ⏳ #32 programs-as-data: in-language `load()` + demos — the homoiconic principle made visible. (a) **Demo test landed** (`chrysalis/tests/programs_as_data.rs`): `grow-divide-unbounded.ys` runs identically as a Program (parsed source) AND as a Document VALUE (`document_of` → JSON → `from_str` → `run_document`); the three representations all reach the same final state. (b) **REMAINING**: surface `load(path)` to `.ys` itself — today the round-trip is a Rust-only path; an in-language `from io import load` would let a `.ys` program build / inspect / run another program at runtime. Pairs with #30 (entities as values) + tier-2 (process bodies as first-class values). (c) **STRETCH**: AST-as-value — expose the AST itself as a tree-of-maps so meta-programming is just term construction over the AST shape. The Document gives the *compiled* program-as-data today; the AST shape is the *uncompiled* program-as-data.
