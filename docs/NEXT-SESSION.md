...
- ⏳ #50 unify the import forms — ONE form: `from <dotted.path> import <Name, …>`. Drop `import N from 'path'` (`Def::Import`: whole-file merge, vestigial Name, invisible defs — you can't tell where a later `Rk4` came from). Extend `Def::Use` to a dotted path resolved **std-module-first** (`integrators`/`core`/…) **then file** (dots→dirs + `.ys`, relative to the importer; retire the tests' rewrite-to-absolute hack for proper relative resolution). **Explicit named selection** — strong form: only the named defs are usable at the import site (strict; transitive deps pulled along for compile), so every control traces to its import. Rewire the ~6 demos using `import … from '…'` (agreement/integrator-comparison/cme-gillespie/external/agreement-engines/schlogl-engines) + the 2 tests. The surface-syntax slice of the #46 unity (user: "explicit selection; one form; dotted not string").
