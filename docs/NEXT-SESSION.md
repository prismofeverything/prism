...
- 🧩 #43 cross-composite reactor — **FIRST SLICE DONE 2026-06-08b** (the CRUX delta-returning reactor + engine per-tick reactor + auto-detect); the #40 link-graph surface matcher + chrysalis wiring + distributed form remain. The arc:
  ...
  - ⏳ **REMAINING**: (a) the #40 link-graph surface MATCHER — remove the two grammar restrictions (redex head may be `?x`; port target may bind `?v`), lower `?x ~{p: ~e}` to a shared-link match; (b) chrysalis surface wiring — a control → `CrossCompositeReactor` (e.g. a cross-composite `BRS` mode) + a `.ys` demo (two cells bond over a shared `~e`); (c) the distributed form via #42 Bigraph-typed updates over a real `rest:`/`stream:` bridge; (d) LIVE sub-engine composites — make `config.state` the live source so the reactor reaches a RUNNING composite's evolving interior (today the engine test uses inert composite specs — no `address` → not instantiated as sub-engines — to isolate the reactor mechanism from this orthogonal sync).
