...
- 🧩 #32 programs-as-data: in-language `load()` + demos — the homoiconic principle made visible.  
  ...
  - ⏳ **STRETCH** — AST-as-value: expose the AST as a tree-of-maps so meta-programming is *just* term construction over the AST shape. The Document gives the *compiled* program-as-data today; the AST shape is the *uncompiled* form. Pre-req for tier-2 (process bodies as first-class values).
