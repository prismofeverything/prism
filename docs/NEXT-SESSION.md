...
- 🧩 #32 programs-as-data: in-language `load()` + demos — the homoiconic principle made visible.  
  - ✅ **JSON round-trip demo** (`programs_as_data.rs`): `grow-divide-unbounded.ys` runs identically as a Program (parsed source) AND as a Document VALUE (`document_of` → JSON → `from_str` → `run_document`); three representations reach the same final state.  
  - ✅ **`from io import load` + `Document.run(time)`** wired through compile/eval. Native FUNCTION imports now flow through `ResolvedImports::functions` → `Evaluator::imported_functions` → `Expr::Call`'s dispatch (previously a `bare-function import is not supported yet` error). `prelude::load(path) → Value` is also exposed publicly for Rust callers. Tests in `chrysalis/tests/load_in_language.rs`:  
    - `load_returns_program_as_value` — confirms `load(path)` returns `{_type: "Document", schema, state, _source}` as a plain `Value::Map`.  
    - `document_run_dispatches_for_loaded_and_hand_built_alike` — runs BOTH a `load()`'d Document AND a HAND-BUILT `Value::Map` (just map literals + `_source: tick.ys`) through the same `Document.run(5.0)` method dispatcher; both produce `count: 5.0`. The Lisp parallel: `load` ~ `read`, `Document.run` ~ `eval`, map literals are the data-form `cons`/`list`. Programs are values; the host functions that build them have no special privilege over user code building them.  
  - **The `_source` bookkeeping**: a loaded Document carries the source path so `.run` can re-compile and pull in the loaded program's user-defined process factories (which `std_core` alone wouldn't know). Hand-built Documents using only std processes don't need it; loaded ones do until tier-2 lands.  
  - ⏳ **REMAINING**: (a) the bin's entry-rule for trailing scalar/method-chain expressions (`load(p).run(t).count` as a trailing expr) — needed for a pure-`.ys` demo without going through Rust. (b) `Document.run` returns full state (incl. internal process specs); a future variant returns only the OUTPUT FACE so loaded results compose cleanly into outer programs.  
  - ⏳ **STRETCH** — AST-as-value: expose the AST itself as a tree-of-maps so meta-programming is *just* term construction over the AST shape. The Document gives the *compiled* program-as-data today; the AST shape is the *uncompiled* program-as-data. Pre-req for tier-2 (process bodies as first-class values).
