...
/// Render a PLAY runner (`docs/domain-libraries.md` §5): like the native-deps runner, but
/// the `run` body BUILDS the engine (via `chrysalis::runner::build_engine`) and drives it
/// to the `sink` crate's `prelude::run_realtime(engine, seconds)` device for `--time`
/// seconds, instead of `run_command`. The sink crate gets the `realtime` feature; play
/// stays chrysalis-free (a *built* engine is handed to the device — the sink crate needs
/// no language facet).
fn render_play_runner(
    manifest_name: &str,
    parts: &NativeParts,
    sink: &NativeCrate,
    chrysalis_dir: &Path,
    ...
    // [dependencies]: chrysalis + each native crate; the SINK crate enables `realtime`.
    let mut crate_deps = String::new();
    for c in parts.crates() {
        let feats = if c.cargo_name == sink.cargo_name {
            ", features = [\"realtime\"]"
        } else {
            ""
        };
    ...
    let cargo_toml = format!(
        "# GENERATED by `chrysalis` — PLAY runner for package `{name}` (sink: `{sink}`).\n\
         [package]\n\
         name = \"chrysalis-runner-{name}-play\"\n\
         version = \"0.0.0\"\n\
         edition = \"2021\"\n\
         \n\
         [[bin]]\n\
         ...
         [workspace]\n",
        name = manifest_name,
        sink = sink.cargo_name,
        bin = bin_name,
        chrysalis = chrysalis_dir,
    );

    let mut inserts = String::new();
    for (edge, krate) in &parts.deps {
        inserts.push_str(&format!(
            "    native_cores.insert(\"{edge}\".to_string(), {ident}::prelude::core());\n",
            edge = edge,
            ident = krate.ident(),
    ...
    let main_rs = format!(
        "// GENERATED by `chrysalis` — PLAY mode. Resolves the package Core, BUILDS the\n\
         // engine via build_engine, and drives it to the `{sink}` device sink\n\
         // (prelude::run_realtime) for --time seconds — the same engine an offline run builds.\n\
         ...
         \x20   let code = match argv.split_first() {{\n\
         \x20       Some((cmd, rest)) if cmd == \"run\" => run(rest),\n\
         \x20       _ => {{ eprintln!(\"usage: runner run <patch.ys> --play [--time T]\"); 2 }}\n\
         \x20   }};\n\
         \x20   std::process::exit(code);\n\
         }}\n\
         \n\
         fn play_seconds(args: &[String]) -> f64 {{\n\
         \x20   let mut it = args.iter();\n\
         \x20   while let Some(a) = it.next() {{\n\
         \x20       if a == \"--time\" {{\n\
         \x20           if let Some(v) = it.next() {{ if let Ok(s) = v.parse::<f64>() {{ return s; }} }}\n\
         \x20       }}\n\
         \x20   }}\n\
         \x20   5.0\n\
         }}\n\
         ...
         \x20   let entry = match args.iter().find(|a| !a.starts_with(\"--\")) {{\n\
         \x20       Some(p) => p.clone(),\n\
         \x20       None => {{ eprintln!(\"chrysalis play: no .ys patch in args\"); return 2; }}\n\
         \x20   }};\n\
         \x20   let manifest = match chrysalis::manifest::Manifest::find(&entry) {{\n\
         \x20       Some(m) => m,\n\
         \x20       None => {{ eprintln!(\"chrysalis play: no `def package` manifest above {{entry}}\"); return 1; }}\n\
         \x20   }};\n\
         \x20   let mut native_cores = HashMap::new();\n\
         {inserts}\
         \x20   let dir = std::path::Path::new(&entry).parent().map(|d| d.to_path_buf()).unwrap_or_else(|| std::path::PathBuf::from(\".\"));\n\
         \x20   let base_modules = chrysalis::prelude::std_modules_at(Some(dir.clone()));\n\
         \x20   let resolution = match chrysalis::resolver::resolve_with_natives(&manifest, base_modules, native_cores) {{\n\
         \x20       Ok(r) => r,\n\
         \x20       Err(e) => {{ eprintln!(\"chrysalis play: resolving dependencies: {{e}}\"); return 1; }}\n\
         \x20   }};\n\
         \x20   let src = match std::fs::read_to_string(&entry) {{\n\
         \x20       Ok(s) => s,\n\
         \x20       Err(e) => {{ eprintln!(\"chrysalis play: read {{entry}}: {{e}}\"); return 1; }}\n\
         \x20   }};\n\
         \x20   let program = match chrysalis::parse::parse_program_in(&src, &dir) {{\n\
         \x20       Ok(p) => p,\n\
         \x20       Err(e) => {{ eprintln!(\"chrysalis play: parse {{entry}}: {{e}}\"); return 1; }}\n\
         \x20   }};\n\
         \x20   let engine = match chrysalis::runner::build_engine(&program, resolution.core, resolution.modules, &std::collections::BTreeMap::new()) {{\n\
         \x20       Ok(e) => e,\n\
         \x20       Err(e) => {{ eprintln!(\"chrysalis play: build engine: {{e:?}}\"); return 1; }}\n\
         \x20   }};\n\
         \x20   match {sink_ident}::prelude::run_realtime(engine, play_seconds(args)) {{\n\
         \x20       Ok(()) => 0,\n\
         \x20       Err(e) => {{ eprintln!(\"chrysalis play: {{e}}\"); 1 }}\n\
         \x20   }}\n\
         }}\n",
        sink = sink.cargo_name,
        sink_ident = sink.ident(),
        inserts = inserts,
    );
...
/// PLAY mode: render + build + exec a runner that drives the engine to the package's
/// declared `sink:` device (`docs/domain-libraries.md` §5) for `--time` seconds, instead
/// of the offline `run_command`. The sink names a native dependency whose
/// `prelude::run_realtime(engine, seconds)` is the output functor. A SEPARATE gen-dir
/// (`<name>-play`) caches the realtime runner apart from the offline one.
fn run_play(
    ...
    let sink_name = match &manifest.sink {
        Some(s) => s,
        None => {
            eprintln!(
                "chrysalis codegen: `--play` needs a `sink:` in project.ys — package `{}` \
                 declares no output boundary.",
                manifest.name
            );
            return 1;
        }
    };
    let sink = match find_sink(sink_name, parts) {
        Some(c) => c,
        None => {
            eprintln!(
                "chrysalis codegen: sink `{sink_name}` names no native dependency of `{}`.",
                manifest.name
            );
            return 1;
        }
    };
    let layout = match Layout::resolve(&format!("{}-play", manifest.name)) {
        Ok(l) => l,
        Err(e) => {
            eprintln!("chrysalis codegen: {e}");
            return 1;
    ...
    let (cargo_toml, main_rs) =
        render_play_runner(&manifest.name, parts, sink, &layout.chrysalis_dir, &layout.bin_name);
    build_and_exec(&layout, &cargo_toml, &main_rs, subcommand, args)
}

/// Find the native crate a `sink:` names (a native-dependency edge).
fn find_sink<'a>(sink: &str, parts: &'a NativeParts) -> Option<&'a NativeCrate> {
    parts.deps.iter().find(|(edge, _)| edge == sink).map(|(_, c)| c)
}

/// Build (cached) + exec a generated runner, forwarding `subcommand` + `args`. Returns
/// the runner's exit code. Shared by the offline + play paths.
fn build_and_exec(
    layout: &Layout,
    cargo_toml: &str,
    main_rs: &str,
...
mod tests {
    ...
    // ── PLAY sink (docs/domain-libraries.md §5) ──

    #[test]
    fn find_sink_resolves_a_native_edge() {
        let parts = NativeParts {
            own: None,
            deps: vec![
                ("audio".into(), nc("prism-audio", "/c/audio")),
                ("fx".into(), nc("prism-fx", "/c/fx")),
            ],
        };
        assert_eq!(find_sink("audio", &parts).unwrap().cargo_name, "prism-audio");
        assert_eq!(find_sink("fx", &parts).unwrap().cargo_name, "prism-fx");
        assert!(find_sink("nope", &parts).is_none());
    }

    #[test]
    fn renders_a_play_runner() {
        // A sink-declaring package (synth → sink `audio`) renders a PLAY runner: the sink
        // crate gets `realtime`, and the body builds the engine + drives it to the device.
        let parts = NativeParts {
            own: None,
            deps: vec![("audio".into(), nc("prism-audio", "/crates/prism-audio"))],
        };
        let sink = &parts.deps[0].1;
        let (cargo, main) = render_play_runner(
            "synth",
            &parts,
            sink,
            Path::new("/cz"),
            "chrysalis_runner_synth_play",
        );
        // The sink crate enables `realtime`; chrysalis is linked; standalone.
        assert!(cargo.contains(
            "prism-audio = { path = \"/crates/prism-audio\", features = [\"realtime\"] }"
        ));
        assert!(cargo.contains("chrysalis = { path = \"/cz\" }"));
        assert!(cargo.contains("[workspace]"));
        // The body BUILDS the engine + drives it to the sink — NOT offline `run_command`.
        assert!(main.contains("chrysalis::runner::build_engine"));
        assert!(main.contains("prism_audio::prelude::run_realtime(engine, play_seconds(args))"));
        assert!(main
            .contains("native_cores.insert(\"audio\".to_string(), prism_audio::prelude::core());"));
        assert!(!main.contains("run_command"), "play mode does not render offline JSON");
    }
}
