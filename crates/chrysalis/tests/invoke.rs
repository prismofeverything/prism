...
fn run_args(src: &str, pairs: &[(&str, &str)]) -> Result<Value, String> {
    ...
    invoke(&prog, std_registry(), std_methods(), std_modules(), &args(pairs), 1.0)
...
fn binds_config_and_input_renders_output() {
    ...
    let result = out.as_map().and_then(|m| m.get("result")).and_then(|v| v.as_f64());
...
fn missing_required_input_errors() {
    ...
    assert!(err.contains("value"), "error should name the missing input; got: {err}");
}
...
fn output_round_trips_as_input() {
    ...
    let r1 = out1.as_map().and_then(|m| m.get("result")).and_then(|v| v.as_f64()).unwrap();
    // Feed r1 back as `value` (factor 1.0) ⇒ identity, so result == r1.
    let out2 = run_args(SRC, &[("value", &r1.to_string())]).expect("run 2");
    let r2 = out2.as_map().and_then(|m| m.get("result")).and_then(|v| v.as_f64()).unwrap();
    assert_eq!(r1, 8.0);
