...
use chrysalis::stream::{float_record, StreamProcess, StreamProtocol};
...
fn stream_process_drives_a_child_lock_step_over_pipes() {
    ...
    std::fs::write(&echo, "composite Echo ~{n :: Float @ v} ->{out :: Float @ v} (\n  v: n\n)\n")
    ...
    let updates: Vec<Value> = (0..3)
        .map(|n| sp.update(&frame(n as f64), 1.0).into_value().unwrap_or(Value::None))
        .collect();
    ...
    for u in &updates {
        ...
        trajectory.push(acc.get_field("out").and_then(|v| v.as_f64()).unwrap_or(f64::NAN));
    }
    assert_eq!(
        trajectory,
        vec![0.0, 1.0, 2.0],
...
fn engine_steps_a_stream_node_over_pipes() {
    ...
    let driver = Value::tree([
        ("address", Value::from(format!("stream:{}", echo.to_str().unwrap()).as_str())),
        ("config", Value::map()),
    ...
    let state =
        Value::tree([("n", Value::float(5.0)), ("total", Value::float(0.0)), ("driver", driver)]);
