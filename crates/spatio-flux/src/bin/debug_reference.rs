...
fn main() {
    ...
    eprintln!("Fields via get_field: {:?}", state.get_field("fields").map(|v| v.type_name()));
    eprintln!("Fields via as_map: {:?}", state.as_map().map(|m| m.len()));

    // Check glucose specifically
    let glc = state.get_field("fields")
        .and_then(|f| f.get_field("glucose"));
    eprintln!("glucose field: {:?}", glc.map(|v| v.type_name()));

    let glc_val = state.get_path(&[
        "fields".into(), "glucose".into(), "5".into(), "5".into()
    ]);
    ...
    let state = engine.state();
    let glc_after = state.get_path(&[
        "fields".into(), "glucose".into(), "5".into(), "5".into()
    ]);
    eprintln!("After 1s: glucose[5][5] = {:?}", glc_after);

    let mass = state.get_field("particles")
