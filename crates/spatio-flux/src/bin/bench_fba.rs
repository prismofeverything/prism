...
fn bench_model(name: &str, n_solves: usize) {
    ...
    println!(
        "{name:<15} | {n_mets:>4} mets | {n_rxns:>5} rxns | load {load_ms:>6.1}ms | build {build_us:>5}us | cold {cold_us:>6}us | avg {avg_us:>8.1}us ({n_solves} solves) | obj={obj:.4}"
    );
}
